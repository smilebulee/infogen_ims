        ...
        try:
            with mysql_con.cursor(pymysql.cursors.DictCursor) as cursor:
                ...
                sql = "SELECT  A.EMP_EMAL_ADDR "\
                    ...
                    + "       ,CONCAT(SUBSTRING(DATE_SUB(STR_TO_DATE(A.ALL_WRK_TM, '%H%i'), INTERVAL (A.REST_TM+A.DINN_REST_TM) MINUTE) ,1,2), ':', SUBSTRING(DATE_SUB(STR_TO_DATE(A.ALL_WRK_TM, '%H%i'), INTERVAL (A.REST_TM+A.DINN_REST_TM) MINUTE) ,4,2)) AS ALL_WRK_TM "\
        ...
        try:
            with mysql_con.cursor(pymysql.cursors.DictCursor) as cursor:
                sql = "SELECT CASE WHEN NVL(B.EMP_EMAL_ADDR, '') = '' THEN A.EMP_EMAL_ADDR" \
                      ...
                      "            ELSE ''" \
                      "             END HLDY_WRK_YN" \
                      "      ,CASE WHEN NVL(B.PTO_KD_CD, '') = '01' THEN CONCAT(SUBSTRING(DATE_SUB(STR_TO_DATE(A.ALL_WRK_TM, '%H%i'), INTERVAL (A.REST_TM+A.DINN_REST_TM) MINUTE) ,1,2), ':', SUBSTRING(DATE_SUB(STR_TO_DATE(A.ALL_WRK_TM, '%H%i'), INTERVAL (A.REST_TM+A.DINN_REST_TM) MINUTE) ,4,2))" \
