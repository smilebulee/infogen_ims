        ...
        try:
            with mysql_con.cursor(pymysql.cursors.DictCursor) as cursor:
                sql = "SELECT CASE WHEN NVL(B.EMP_EMAL_ADDR, '') = '' THEN A.EMP_EMAL_ADDR" \
                      ...
                      "		 ,CONCAT(SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,1,2), ':', SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,4,2)) ALL_WRK_TM" \
                      ...
                      "            ELSE ''" \
                      "             END HLDY_WRK_YN" \
                      "      ,CASE WHEN NVL(B.PTO_KD_CD, '') = '01' THEN CONCAT(SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,1,2), ':', SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,4,2))" \
                      "            WHEN NVL(B.PTO_KD_CD, '') = '02' THEN CONCAT(SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,1,2), ':', SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,4,2))" \
                      "            ELSE ''" \
                      "             END PTO_KD_YN" \
                      "      ,CASE WHEN A.APRV_STUS = '1' THEN 'Y'" \
                      "            ELSE 'N'" \
        ...
        try:
            with mysql_con.cursor(pymysql.cursors.DictCursor) as cursor:
                sql = "SELECT CASE WHEN NVL(B.EMP_EMAL_ADDR, '') = '' THEN A.EMP_EMAL_ADDR" \
                      ...
                      "	         FROM TB_EMP_MGMT C" \
                      "	        WHERE C.EMP_EMAIL = NVL(A.EMP_EMAL_ADDR, B.EMP_EMAL_ADDR)) OCEM_NAME" \
                      "      ,(SUM(SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,1,2))" \
                      "       + FLOOR(SUM(SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,4,2))/60)) ALL_WRK_TM_T" \
                      "      ,MOD(SUM(SUBSTRING(STR_TO_DATE(A.ALL_WRK_TM, '%H%i') ,4,2)),60) ALL_WRK_TM_M" \
